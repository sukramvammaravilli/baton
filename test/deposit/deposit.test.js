...
describe("Deposit Module", () => {
  ...
  test("DEPOSIT_007 Invalid Account", async () => {
    ...
    expect(response.body.error).toBe("Invalid Request, Account is not active");
  });
  ...
  test("DEPOSIT_008 Empty Account", async () => {
    ...
    expect(response.body.error).toBe(
      "Invalid request, Missing Parameter accountNo",
    );
  });
  ...
  test("DEPOSIT_009 Zero Amount", async () => {
    ...
    expect(response.statusCode).toBe(400);
    expect(response.body.error).toBe(
      "Invalid request, Invalid deposit amount entered ",
    );
  });
  ...
  test("DEPOSIT_011 Invalid Currency", async () => {
    ...
    expect(response.statusCode).toBe(400);
    expect(response.body.error).toBe(
      "Invalid Request, Invalid Exchange Currency",
    );
  });

  test("DEPOSIT_012 Missing Token", async () => {
    const response = await request(app).post("/api/dashboard/deposit").send({
      accountNo: "ACC99999",
      amount: 100,
      currency: "USD,US Dollar",
    });
